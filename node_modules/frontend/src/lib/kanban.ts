...
export type ApiBoardData = {
  columns: Array<{
    id: string;
    title: string;
    cards: Array<Card & { description?: string }>;
  }>;
};
...
export const toApiBoard = (board: BoardData): ApiBoardData => ({
...
export const fromApiBoard = (apiBoard: ApiBoardData): BoardData => {
  if (!apiBoard.columns.some((column) => column.cards.length > 0)) {
    return initialData;
  }

  const cards = Object.fromEntries(
    apiBoard.columns.flatMap((column) =>
      column.cards.map((card) => [
        card.id,
        { ...card, details: card.details || card.description || "" },
      ])
    )
  ) as Record<string, Card>;

  return {
    columns: apiBoard.columns.map((column) => ({
      id: column.id,
      title: column.title,
